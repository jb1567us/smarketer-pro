import os
import sys
from .config_loader import DirectiveConfig
...
class LLMFactory:
    ...
    def get_provider(cls):
        ...
        llm_config = DirectiveConfig.get('llm', {})
        ...
        if mode == 'router':
            ...
            if not candidates:
                ...
                for candidate in candidates:
                    ...
                    tier = candidate.get('tier', 'economy') # Tagging
                    
                    if p_name:
                         try:
                             instance = cls._create_provider(p_name, m_name)
                             providers.append({'instance': instance, 'tier': tier})
    ...
    def _create_provider(provider_name, model_name=None):
        if provider_name == 'openai':
            ...
            base_url = DirectiveConfig.get('llm', {}).get('ollama_base_url', 'http://localhost:11434')
            return OllamaProvider(model, base_url)
            
        elif provider_name == 'openrouter':
            api_key = os.getenv("OPENROUTER_API_KEY")
            if not api_key: raise ValueError("Missing OPENROUTER_API_KEY")
        ...
        # -- New Providers --
            ...
            if not api_key: raise ValueError("Missing MISTRAL_API_KEY")
            ...
            if not api_key: raise ValueError("Missing HUGGINGFACE_API_KEY")
            ...
            if not api_key: raise ValueError("Missing CEREBRAS_API_KEY")
            ...
            if not api_key: raise ValueError("Missing COHERE_API_KEY")
            model = model_name or 'command-r'
            return CohereProvider(api_key, model)

        elif provider_name == 'github_models':
            api_key = os.getenv("GITHUB_TOKEN")
            if not api_key: raise ValueError("Missing GITHUB_TOKEN")
            ...
            if not api_key or not account_id: raise ValueError("Missing CLOUDFLARE_API_KEY or CLOUDFLARE_ACCOUNT_ID")
