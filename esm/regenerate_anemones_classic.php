<?php
// regenerate_anemones_classic.php
...
if (!$artwork)
    die("Anemones not found");

// 3. Embedded Template (NO WP WRAPPERS)
$template = <<<HTML
...
<style>
    ...
    .artwork-hero-image {
        ...
        @media (min-width: 800px) { max-width: 800px; }
    }
    ...
    .btn-saatchi:hover { background-color: #333; transform: translateY(-1px); }
    ...
    .detail-label {
        font-size: 0.7rem; text-transform: uppercase; letter-spacing: 1.5px; color: #1a1a1a; margin-bottom: 0.35rem; font-weight: 700;
    }
    .detail-value {
        font-weight: 400; color: #1a1a1a; font-size: 1.1rem; font-family: 'Playfair Display', serif;
    }

    /* Content Sections */
    .artwork-description { font-size: 1.05rem; margin-bottom: 3rem; color: #4a4a4a; }
    .section-divider { height: 1px; background: #e0e0e0; margin: 3rem 0; border: none; }

    /* Designer Specs */
    .designer-specs-premium {
        border: 1px solid #e0e0e0; padding: 2rem; border-radius: 8px; background: #fff; position: relative; overflow: hidden;
    }
    .designer-specs-premium::before {
        content: "TRADE ONLY"; position: absolute; top: 1rem; right: 1rem; font-size: 0.6rem; font-weight: 700; border: 1px solid #1a1a1a; padding: 0.25rem 0.5rem; border-radius: 2px;
    }
    .specs-title { font-family: 'Playfair Display', serif; font-size: 1.5rem; margin-bottom: 1.5rem; }
...
<div class="artwork-page-container">
    ...
    <img src="{{IMAGE_URL}}" alt="{{TITLE}}" class="artwork-hero-image">
    <div class="artwork-actions">
        <a href="{{SAATCHI_URL}}" class="btn-premium btn-saatchi">Purchase on Saatchi Art</a>
        <a href="/trade" class="btn-premium btn-trade">Request Trade Pricing</a>
    </div>
    <div style="margin-top: 3rem; padding-top: 2rem; border-top: 1px solid rgba(0,0,0,0.1);">
        ...
        <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 1rem; font-size: 0.95rem; color: #555;">
            <div><strong>Dimensions:</strong></div><div>{{DIMENSIONS}}</div>
            <div><strong>Styles:</strong></div><div>{{STYLES}}</div>
            <div><strong>Mediums:</strong></div><div>{{MEDIUMS_DETAILED}}</div>
        </div>
    </div>
    ...
    <div class="designer-specs-premium">
        ...
        <ul class="specs-list">
            ...
            <li><span class="check-icon">✓</span> <strong>Weight:</strong> 5 lbs</li>
            <li><span class="check-icon">✓</span> <strong>Framing:</strong> {{FRAME}}</li>
            ...
            <li style="border-bottom: none; padding-top: 1rem; margin-top: 0.5rem; border-top: 1px dashed #eee;">
                <span style="font-size: 0.75rem; color: #999; text-transform: uppercase;">Tags:</span>
                <span style="font-size: 0.85rem; color: #666;">{{TAGS}}</span>
            </li>
        </ul>
        <div style="margin-top: 1.5rem; display: flex; gap: 1rem;">
            <a href="/downloads/caviar-spec-sheet.pdf" style="font-size:0.9rem; color:#666; text-decoration:none;">Download Spec Sheet</a>
            <a href="/downloads/caviar-high-res.zip" style="font-size:0.9rem; color:#666; text-decoration:none;">High-Res Images</a>
        </div>
    </div>
    <div style="display: flex; align-items: center; gap: 1rem; margin-top: 2rem;">
        <div><h4 style="margin:0;">Elliot Spencer Morgan</h4><p style="margin:0;">Austin, TX</p></div>
        <a href="/about" style="margin-left:auto;">View Profile →</a>
    </div>
</div>
HTML;
...
if ($row) {
    ...
    echo "✅ Regenerated Anemones CLASSIC (ID {$row->ID}). New Length: " . strlen($content);
    if (function_exists('w3tc_flush_all'))
        w3tc_flush_all();
    if (function_exists('wp_cache_flush'))
        wp_cache_flush();
} else {
    echo "❌ Anemones not found in DB.";
}
?>