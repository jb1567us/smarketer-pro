...
foreach ($pages as $page) {
    ...
    // --- STEP 1: CLEANUP ---
    // Perform cleanup if mode is 'clean' or 'inject' (or dry run analysis)
    // But actual save only happens if mode matches

    // Remove old "artwork-downloads" div
    $content = preg_replace('/<div class=[\'"]artwork-downloads[\'"].*?<\/div>/s', '', $content);
    // Remove loose ugly buttons (broad match)
    $content = preg_replace('/<a [^>]*class=[\'"]btn btn-(spec|zip)[\'"][^>]*>.*?<\/a>/s', '', $content);
    $content = preg_replace('/<br>\s*<a [^>]*>.*?Download Spec Sheet.*?<\/a>/is', '', $content);
    // Remove "DOWNLOAD HIGH RES IMAGES" block
    $content = preg_replace('/<a[^>]*style=[^>]*background[^>]*>.*?DOWNLOAD HIGH RES.*?<\/a>/is', '', $content);

    // Remove RAW CSS visible on page (Fix for /* Font Imports */...)
    // Matches from "/* Font Imports */" up to the next opening tag "<"
    if (strpos($content, '/* Font Imports */') !== false && strpos($content, '<style>/* Font Imports */') === false) {
        $content = preg_replace('/\/\* Font Imports \*\/.*?(?=<)/s', '', $content);
        echo "<span class='removed'>Removed RAW CSS. </span>";
    }

    // Remove UNSTYLED "Designer Specifications" block (Header + List)
    // This targets the broken state seen on Animal Kingdom
    ...
    if (strlen($content) != $orig_len) {
        ...
        // If mode is explicit CLEAN, apply changes and skip injection
        if ($mode === 'clean') {
            wp_update_post(['ID' => $page->ID, 'post_content' => $content]);
            echo " <strong>[SAVED CLEAN]</strong></div>";
            continue;
        }
    } else {
        // Nothing to clean
