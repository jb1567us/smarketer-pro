...
class WordPressPortfolioGenerator:
    def __init__(self):
        ...
        self.artworks = []
        self.artist_info = {"name": "Elliot Spencer Morgan", "bio": "", "statement": ""}
    ...
    def get_artwork_urls(self):
        """Return the list of artwork URLs (reduced to 4 for testing)"""
        return [
    ...
    def extract_artist_info(self):
        ...
        try:
            ...
            if not self.artist_info["bio"]:
                self.artist_info[
                    "bio"
                ] = "Elliot Spencer Morgan is a contemporary artist working across multiple mediums including painting, sculpture, collage, and installation. Their work explores the intersection of organic forms and structured compositions."

            # Artist statement
            self.artist_info[
                "statement"
            ] = "My work is an exploration of the relationship between natural forms and constructed environments, blending organic shapes with geometric precision to create visual dialogues about our place in the modern world."
            ...
            self.artist_info[
                "bio"
            ] = "Elliot Spencer Morgan is a contemporary artist working across multiple mediums including painting, sculpture, collage, and installation."
            self.artist_info[
                "statement"
            ] = "My work explores the relationship between natural forms and constructed environments."
    ...
    def extract_medium(self, soup):
        ...
        # Look for medium in details section
    ...
    def extract_year(self, soup):
        ...
        year_pattern = r"Year Created:\s*(\d{4})"
        text = soup.get_text()
        match = re.search(year_pattern, text, re.IGNORECASE)
        return match.group(1) if match else "Year not specified"
    ...
    def extract_subject(self, soup):
        ...
        return match.group(1).strip() if match else "Subject not specified"

    def extract_styles(self, soup):
        """Extract styles"""
        styles_pattern = r"Styles:\s*(.+)"
        text = soup.get_text()
        match = re.search(styles_pattern, text, re.IGNORECASE)
        return match.group(1).strip() if match else "Styles not specified"

    def extract_rarity(self, soup):
        """Extract rarity information"""
        rarity_pattern = r"Rarity:\s*(.+)"
        text = soup.get_text()
        match = re.search(rarity_pattern, text, re.IGNORECASE)
        return match.group(1).strip() if match else "Rarity not specified"

    def extract_ready_to_hang(self, soup):
        """Extract ready to hang information"""
        ready_pattern = r"Ready To Hang:\s*(.+)"
        text = soup.get_text()
        match = re.search(ready_pattern, text, re.IGNORECASE)
        return match.group(1).strip() if match else "Not specified"

    def extract_framing(self, soup):
        """Extract framing information"""
        frame_pattern = r"Frame:\s*(.+)"
        text = soup.get_text()
        match = re.search(frame_pattern, text, re.IGNORECASE)
        return match.group(1).strip() if match else "Not specified"

    def extract_authenticity(self, soup):
        """Extract authenticity information"""
        auth_pattern = r"Authenticity:\s*(.+)"
        text = soup.get_text()
        match = re.search(auth_pattern, text, re.IGNORECASE)
        return match.group(1).strip() if match else "Not specified"

    def extract_packaging(self, soup):
        """Extract packaging information"""
        pack_pattern = r"Packaging:\s*(.+)"
        text = soup.get_text()
        match = re.search(pack_pattern, text, re.IGNORECASE)
        return match.group(1).strip() if match else "Not specified"
    ...
    def generate_wordpress_xml(self):
        ...
        for i, cat in enumerate(categories, 1):
            ...
            term_id = ET.SubElement(term, "wp:term_id")
            term_id.text = str(i)

            term_taxonomy = ET.SubElement(term, "wp:term_taxonomy")
            term_taxonomy.text = "category"

            term_slug = ET.SubElement(term, "wp:term_slug")
            term_slug.text = cat.lower()

            term_name = ET.SubElement(term, "wp:term_name")
            term_name.text = cat

        # Add author information
        author = ET.SubElement(channel, "wp:author")

        author_id = ET.SubElement(author, "wp:author_id")
        author_id.text = "1"

        author_login = ET.SubElement(author, "wp:author_login")
        author_login.text = "admin"

        author_email = ET.SubElement(author, "wp:author_email")
        author_email.text = "admin@lookoverhere.xyz"

        author_display_name = ET.SubElement(author, "wp:author_display_name")
        author_display_name.text = "admin"

        author_first_name = ET.SubElement(author, "wp:author_first_name")
        author_first_name.text = "Admin"

        author_last_name = ET.SubElement(author, "wp:author_last_name")
        ...
        for i, artwork in enumerate(self.artworks, 1):
            if artwork.get("images"):
                ...
                content.text = CDATA(self.generate_wordpress_content(artwork))

                # Excerpt
                excerpt = ET.SubElement(item, "excerpt:encoded")
                excerpt_text = (
                    artwork["description"][:200] + "..."
                ...
                category = ET.SubElement(
                    ...
                    "category",
                    domain="category",
                    nicename=self.determine_category(artwork["medium"]).lower(),
                )
                ...
                for key, value in meta_fields.items():
                    ...
                    meta_key = ET.SubElement(postmeta, "wp:meta_key")
                    meta_key.text = key

                    meta_value = ET.SubElement(postmeta, "wp:meta_value")
                    meta_value.text = str(value)

        # Add About and Contact pages
        pages_content = self.create_pages_xml()
    ...
    def create_post_xml_string(self, artwork, post_id):
        ...
        excerpt_text = (
            artwork["description"][:200] + "..." if artwork["description"] else ""
        )

        post_xml = f"""
        <item>
            <title>{artwork['title']}</title>
            <link>https://lookoverhere.xyz/elliot/artwork/{self.sanitize_filename(artwork['title'])}</link>
    ...
    def create_pages_xml(self):
        ...
        about_page = self.create_page_xml(
            "About", about_content, len(self.artworks) + 1, "about"
        )
        pages.append(about_page)

        # Contact page
        contact_page = self.create_page_xml(
            "Contact", contact_content, len(self.artworks) + 2, "contact"
        )
        pages.append(contact_page)

        return pages

    def create_page_xml(self, title, content, post_id, slug):
        """Create XML for a page"""
        ...
        menu_order = ET.SubElement(item, "wp:menu_order")
        menu_order.text = str(
            post_id - len(self.artworks)
        )  # 1 for About, 2 for Contact
    ...
    def generate_wordpress_content(self, artwork):
        ...
        <div class="artwork-detail-page">
            <div class="artwork-gallery">
                {"".join([f'<img src="{img["url"]}" alt="{artwork["title"]}" class="artwork-image" />' for img in artwork.get("wordpress_images", [])])}
            </div>
