...
class WordPressPortfolioGenerator:
    ...
    def create_directory_structure(self):
        ...
        self.theme_dir = os.path.join(self.base_dir, "theme", "esm-portfolio")  # Theme folder with proper name
    ...
    def extract_main_image(self, soup):
        ...
        # Try large image tags
        img_selectors = [
            'img[data-testid="artwork-image"]',
            '.artwork-image',
            '.main-image',
            'img[class*="artwork"]',
            'img[src*="cloudfront.net"]',  # Saatchi uses CloudFront
        ]
    ...
    def download_image(self, artwork):
        ...
        try:
            ...
            # Try to get proper extension from URL or content type
    ...
    def generate_theme_files(self):
        """Generate complete WordPress theme with all required files"""
        # style.css - Must be first file with proper header
        style_content = f"""/*
...
"""
        ...
        # screenshot.png - Theme screenshot (optional but recommended)
        # Create a simple placeholder screenshot
        from PIL import Image, ImageDraw, ImageFont
        try:
            # Create a simple screenshot image
            img = Image.new('RGB', (1200, 900), color='#2c3e50')
            ...
            d.text((100, 100), "ESM Portfolio", fill=(255, 255, 255), font=font)
            d.text((100, 200), "Elliot Spencer Morgan", fill=(255, 255, 255), font=font)
            
            img.save(os.path.join(self.theme_dir, "screenshot.png"))
            print("✅ Created theme screenshot")
        except Exception as e:
            print(f"⚠️  Could not create screenshot: {e}")
    ...
    def generate_import_instructions(self):
        ...
        ## Important Notes:
        - The theme folder is named `esm-portfolio` - zip this specific folder
        - All template files are included (index.php, header.php, footer.php, etc.)
        - Theme meets WordPress theme requirements
        """
    ...
    def run(self):
        ...
        print(f"\\n💡 Important: Zip the 'esm-portfolio' folder (not the parent 'theme' folder) for installation")
