...
class WordPressPortfolioGenerator:
    ...
    def generate_wordpress_xml(self):
        ...
        for i, artwork in enumerate(self.artworks, 1):
            if artwork.get('wordpress_image'):
                ...
                # Custom fields for Saatchi URL
                postmeta = ET.SubElement(item, "{%s}postmeta" % nsmap['wp'])
                ET.SubElement(postmeta, "{%s}meta_key" % nsmap['wp']).text = "saatchi_url"
                ET.SubElement(postmeta, "{%s}meta_value" % nsmap['wp']).text = artwork['url']
    ...
    def generate_theme_files(self):
        """Generate complete WordPress theme with updated homepage styling"""
        # Generate navigation menu HTML
        navigation_menu = self.generate_navigation_menu()
        
        # style.css - Updated for 3 columns and titles below images
        style_content = f"""/*
...
/* Artwork Grid - 3 columns for homepage */
...
.artwork-image {{
    width: 100%;
    height: 250px; /* Reduced height for 3-column layout */
    object-fit: cover;
    display: block;
}}
...
"""
        ...
        # front-page.php - Homepage template with 3-column grid
...
"""
        ...
        # Update the homepage content generation to use the new class
        # We need to modify the generate_homepage_content method to use homepage-artwork-grid
        # Let's update the method above to use the correct class
...
Key Features:
- Homepage with 3 artwork thumbnails per row
- Artwork titles displayed below images (no hover)
- Category pages showing filtered artwork posts
...
"""
    ...
    def generate_homepage_content(self):
        """Generate homepage content with all artwork thumbnails - 3 per row with updated class"""
        homepage_content = f'''
        <div class="homepage-hero">
            <h1>Elliot Spencer Morgan</h1>
            <p class="tagline">Contemporary Art Portfolio</p>
