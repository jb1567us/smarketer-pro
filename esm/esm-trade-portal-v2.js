...
const ESM_Visualizer = {
    ...
    switchTab: function (tab) {
        ...
        const tabs = { 'controls': 0, 'room': 1, 'lighting': 2, 'camera': 3 };
    ...
    captureRoom: function () {
        console.log("Capture clicked");
        const video = document.getElementById('ar-video');
        if (!video) { alert("Video element missing"); return; }
        if (!video.srcObject) { alert("Camera stream not found on video element"); return; }
        console.log("Video dimensions:", video.videoWidth, video.videoHeight);
        if (video.videoWidth === 0) { alert("Video has 0 width, wait for stream to load"); return; }
        ...
        flash.style.position = 'fixed'; flash.style.inset = '0'; flash.style.background = 'white'; flash.style.zIndex = '3000';
        document.body.appendChild(flash);
        setTimeout(() => { flash.style.opacity = '0'; flash.style.transition = 'opacity 0.2s'; }, 50);
        setTimeout(() => { flash.remove(); }, 250);

        const dataUrl = canvas.toDataURL('image/jpeg', 0.85);
        this.setRoomImage(dataUrl);
        this.closeAR();
        this.switchTab('controls'); // Switch back to controls to place art
    },
    ...
    startCamera: async function () {
        try {
            ...
            document.getElementById('camera-error-msg').textContent = "Camera error: " + (e.message || "Access denied");
