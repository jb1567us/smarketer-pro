...
Description: Registers [esm_artwork_layout] shortcode to render the single-source-of-truth artwork page layout.
Version: 1.0
Author: ESM Dev
*/
...
class ESM_Artwork_Template
{
    ...
    {
        add_shortcode('esm_artwork_layout', [$this, 'render_layout']);
        $this->load_data();
    }
    ...
    private function load_data()
    {
        ...
        // Fallback to checking root if not in mu-plugins/subdir
        if (!file_exists($file))
    ...
    public function render_layout($atts)
    {
        // Get current post
        $post = get_post();
        if (!$post)
        ...
        $data = isset($this->data_map[$key]) ? $this->data_map[$key] : [];
        ...
        // Description Logic (Constructed since we lack a direct field)
        ...
        // --- 2. CSS STYLES ---
        // Scoped to .esm-master-layout to prevent leaks
        $css = <<<CSS
        <style>
...
CSS;
    ...
    {
        // Simple file check first
        foreach ($candidates as $c) {
            if (file_exists($dir . $c))
                return $c;
        }

        // Case-insensitive check (Windows style but manual for Linux consistency)
        // Only run this if direct match failed, as it's more expensive
        if (!is_dir($dir))
            return false;
        $files = scandir($dir);
