...
<body class="bg-gray-50 text-gray-900 min-h-screen flex flex-col items-center p-8 md:p-24 selection:bg-blue-100">
    ...
    <div class="w-full max-w-2xl grid gap-6">
        ...
        <div class="bg-white p-6 rounded-2xl shadow-xl border border-gray-100">
            ...
            <div class="mt-4">
                <label class="block text-sm font-medium mb-2 text-gray-700">Select Insights to Generate:</label>
                <div class="grid grid-cols-1 md:grid-cols-2 gap-2 text-sm text-gray-600">
                    <label class="flex items-center gap-2"><input type="checkbox" class="section-checkbox rounded text-blue-600" value="personas" checked> <span>Customer Personas</span></label>
                    <label class="flex items-center gap-2"><input type="checkbox" class="section-checkbox rounded text-blue-600" value="journey" checked> <span>Buyer Journey</span></label>
                    <label class="flex items-center gap-2"><input type="checkbox" class="section-checkbox rounded text-blue-600" value="channels" checked> <span>Marketing Channels (incl. B2B Email)</span></label>
                    <label class="flex items-center gap-2"><input type="checkbox" class="section-checkbox rounded text-blue-600" value="content" checked> <span>Content & SEO</span></label>
                    <label class="flex items-center gap-2"><input type="checkbox" class="section-checkbox rounded text-blue-600" value="competition" checked> <span>Competitive Positioning</span></label>
                </div>
            </div>

            <button id="analyzeBtn" onclick="analyzeProduct()"
                class="mt-6 w-full flex items-center justify-center gap-2 bg-blue-600 hover:bg-blue-700 text-white font-semibold py-3 px-6 rounded-xl transition-all disabled:opacity-50 disabled:cursor-not-allowed">
    ...
    <script>
        ...
        async function analyzeProduct() {
            ...
            if (selectedSections.length === 0) {
                alert("Please select at least one insight section to generate.");
                return;
            }
