...
function ttp_register_theme_settings() {
	...
	$fields = array(
		...
		'locality_center' => array( 'label' => 'Locality Center (origin)', 'type' => 'text', 'section' => 'ttp_section_local' ),
	);
	...
	foreach ( $fields as $key => $f ) {
		add_settings_field(
			$key,
			esc_html( $f['label'] ),
			'ttp_render_theme_setting_field',
			'ttp_theme_options',
			esc_attr( $f['section'] ),
			array( 'key' => $key, 'field' => $f )
		);
	}
}
...
function ttp_sanitize_theme_options( $input ) {
	...
	$out['commission_disclosure'] = sanitize_textarea_field( $input['commission_disclosure'] ?? 'Disclosure: We may receive a referral commission if you purchase through a breeder we refer you to.' );

	$out['pickup_address'] = sanitize_text_field( $input['pickup_address'] ?? '3800 Manorwood Rd, Austin, TX 78723' );
	$out['locality_center'] = sanitize_text_field( $input['locality_center'] ?? 'Austin, TX' );

	// Page IDs are set by onboarding; keep if present.
