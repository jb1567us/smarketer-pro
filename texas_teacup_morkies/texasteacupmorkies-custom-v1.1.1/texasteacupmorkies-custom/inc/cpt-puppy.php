...
function ttp_add_puppy_metaboxes() {
	add_meta_box(
		'ttp_puppy_details',
		__( 'Puppy Details', 'ttp' ),
		'ttp_render_puppy_metabox',
		'ttp_puppy',
		'normal',
		'high'
	);
...
function ttp_save_puppy_meta( $post_id ) {
	...
	$status = sanitize_text_field( $_POST['ttp_status'] ?? 'available' );
	if ( ! in_array( $status, array( 'available', 'reserved', 'sold' ), true ) ) { $status = 'available'; }

	$price  = sanitize_text_field( $_POST['ttp_price'] ?? '' );
	$sex    = sanitize_text_field( $_POST['ttp_sex'] ?? '' );
	$color  = sanitize_text_field( $_POST['ttp_color'] ?? '' );
	$gohome = sanitize_text_field( $_POST['ttp_go_home_date'] ?? '' );
