...
function ttp_seed_default_pages_on_activation() {
	...
	if ( empty( $opts['commission_disclosure'] ) ) { $opts['commission_disclosure'] = 'Disclosure: We may receive a referral commission if you purchase through a breeder we refer you to.'; }
	if ( empty( $opts['lead_mode'] ) ) { $opts['lead_mode'] = 'referral'; }

	$price_anchor = $opts['price_anchor'];
	$pickup = $opts['pickup_address'];
	...
	$apply_content =
		...
		. '<li><strong>Pricing is upfront</strong> (typically ' . esc_html( $price_anchor ) . ').</li>'
	...
	$referral_content =
		...
		. '</ul>'
		. '<p><em>' . esc_html( $opts['commission_disclosure'] ) . '</em></p>'
		. '<p>[ttp_lead_cta]</p>';
	...
	$process_content =
		...
		. '</ol>'
		. '<h2>FAQ</h2>'
		. '<h3>Do you deliver?</h3><p>Typically, no. <strong>Pickup is local</strong> at our home in Austin.</p>'
		. '<h3>Why list prices upfront?</h3><p>Because time matters. Boutique, family-raised puppies are a commitment; pricing usually starts around <strong>' . esc_html( $price_anchor ) . '</strong>.</p>'
		. '<h3>Which breeds do you have?</h3><p>Most seasons we have <strong>one breed at a time</strong>. Right now it’s Maltipoms. We keep Morkie info available for future planning.</p>'
	...
	$contact_content =
		...
		. '</div>'
		. '<p>[ttp_map_directions from="Austin, TX" to="' . esc_attr( $pickup ) . '" height="420"]</p>'
		. '<h3>Best way to reach us</h3>'
		. '<ul><li><strong>Text or call</strong> for quick questions.</li><li>Please still submit the form (apply or referral) so we can keep your info organized.</li></ul>'
		. '<p>[ttp_lead_cta]</p>';
	...
	$maltipom_content =
		...
		. '<ul><li><strong>Companion temperament</strong>: sweet, attentive, and people-focused.</li><li><strong>Small size</strong>: a great fit for many Austin households.</li><li><strong>Routine-friendly</strong>: early house-training habits begin right away.</li></ul>'
		. '<h2>Care &amp; grooming</h2>'
		. '<ul><li>Coats vary. Regular brushing prevents tangles and matting.</li><li>A consistent grooming schedule keeps things comfortable long-term.</li></ul>'
		. '<h2>Availability</h2>'
	...
	$morkie_content =
		...
		. '<ul><li><strong>Bright + engaged</strong>: they notice everything and love interaction.</li><li><strong>Small companion size</strong>: great for households that want a close “shadow dog.”</li><li><strong>Personality</strong>: confident, playful, expressive.</li></ul>'
		. '<h2>Care &amp; grooming</h2>'
		. '<ul><li>Regular brushing and a grooming routine keep coats manageable.</li><li>Training and boundaries early help them thrive.</li></ul>'
		. '<h2>Availability</h2>'
	...
	// Local SEO pages (Austin metro) — 5 pages
	$area_intro = 'We\'re an in-home boutique breeder with local pickup in Austin (78723). If you live nearby, this page outlines what pickup looks like and includes directions to us. We typically have one breed at a time—check Available Puppies for the current season.';

	$locals = array(
		array( 'slug' => 'round-rock-puppies', 'title' => 'Maltipom Puppies Near Round Rock, TX (Austin Pickup)', 'from' => 'Round Rock, TX' ),
		array( 'slug' => 'cedar-park-puppies', 'title' => 'Maltipom Puppies Near Cedar Park, TX (Austin Pickup)', 'from' => 'Cedar Park, TX' ),
		array( 'slug' => 'bastrop-puppies', 'title' => 'Maltipom Puppies Near Bastrop, TX (Austin Pickup)', 'from' => 'Bastrop, TX' ),
		array( 'slug' => 'pflugerville-puppies', 'title' => 'Maltipom Puppies Near Pflugerville, TX (Austin Pickup)', 'from' => 'Pflugerville, TX' ),
		array( 'slug' => 'georgetown-puppies', 'title' => 'Maltipom Puppies Near Georgetown, TX (Austin Pickup)', 'from' => 'Georgetown, TX' ),
	);

	foreach ( $locals as $lp ) {
		$from = $lp['from'];
		$content =
			'<p><strong>For families in ' . esc_html( $from ) . ':</strong> ' . esc_html( $area_intro ) . '</p>'
			. '<ul>'
			...
			. '<li><strong>Pricing:</strong> typically ' . esc_html( $price_anchor ) . ' (listed upfront).</li>'
			. '</ul>'
			...
			. '<p class="ttp-muted">Driving directions from the center of ' . esc_html( $from ) . ' to our pickup address.</p>'
			. '<p>[ttp_map_directions from="' . esc_attr( $from ) . '" to="' . esc_attr( $pickup ) . '" height="420"]</p>'
			. '<hr>'
			. '<h2>Local notes</h2>'
		...
		ttp_create_or_update_page( $lp['slug'], $lp['title'], $content );
	}
	...
	if ( empty( $locations[ $primary_loc ] ) ) {
		...
		if ( ! is_wp_error( $menu_id ) ) {
			...
			wp_update_nav_menu_item( $menu_id, 0, array( 'menu-item-title' => 'Available Puppies', 'menu-item-url' => home_url( '/available-puppies/' ), 'menu-item-type' => 'custom', 'menu-item-status' => 'publish' ) );
