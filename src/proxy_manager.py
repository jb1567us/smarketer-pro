...
class ProxyManager:
    ...
    async def update_searxng_config(self):
        """Injects OR REMOVES proxies from SearXNG settings.yml."""
        
        # Priority: Env Var > Default Project Path
        settings_path = os.getenv("SEARXNG_SETTINGS_PATH_HOST")
        ...
        try:
            ...
            # 2. Prepare the new proxies block
            proxy_list = []
            if self.enabled:
            ...
            # If 'outgoing:' doesn't exist, we can't easily append without knowing the structure.
            if "outgoing:" not in content:
                # If it's a fresh file or missing, we can try to append it
                new_block = "\noutgoing:\n  proxies:\n    all://:\n"
                for p in proxy_list:
                    new_block += f"      - {p}\n"
                ...
                # We have 'outgoing:'. We want to find its 'proxies:' subkey.
                # To be robust but comment-safe, we'll replace the block between 'all://:' and the next non-indented line or next subkey.
                
                # Better: Use a simple recursive split or just regex the specific subkey if it exists.
                # Actually, given the user request for "robust", let's use yaml load/dump for JUST the list but preserve the rest.
                
                proxy_yaml_list = "\n".join([f"      - {p}" for p in proxy_list])
                new_proxies_block = f"    all://:\n{proxy_yaml_list}\n"
                
                # Check if 'proxies:' is already under 'outgoing:'
                # Splitting by 'outgoing:'
                parts = content.split("outgoing:", 1)
                pre = parts[0] + "outgoing:\n"
                post = parts[1]
                
                # In post, we look for 'proxies:'
                if "  proxies:" in post:
                    # Replace existing proxies block
                    # We look for 'proxies:' up to the next key with same or less indentation
                    # This is complex. Let's use a simpler heuristic:
                    # Replace everything from 'all://:' until a line that doesn't start with 6 spaces
                    import re
                    pattern = re.compile(r'(\s+all://:\n(\s+-\s+http.*\n?)*)')
                    if pattern.search(post):
                        post = pattern.sub(new_proxies_block, post, count=1)
                    else:
                        # Append under proxies: if all://: wasn't found
                        post = post.replace("  proxies:", f"  proxies:\n{new_proxies_block}", 1)
                else:
                    # Append proxies: under outgoing:
                    post = f"  proxies:\n{new_proxies_block}" + post
                
                final_content = pre + post
    ...
    async def ensure_fresh_proxies(self):
        ...
        is_fresh = self._load_cache()
        if is_fresh and self.proxies:
            print(f"[{self.__class__.__name__}] Proxies are fresh. Loaded {len(self.proxies)} from cache.")
            # Even if fresh, we should ensure SearXNG is updated (in case settings.yml was wiped)
        ...
        if self.proxies:
            ...
            # This ensures that even if SearXNG update fails (e.g. docker down), 
            # we don't discard the valuable harvested proxies.
            self._save_status()
            print(f"[{self.__class__.__name__}] Proxies saved to cache.")
    ...
    def _load_cache(self, max_age_hours=24):
        """Attempts to load proxies from status file if fresh."""
    ...
    def _save_status(self):
        """Update the freshness timestamp and persist proxy list."""
        try:
