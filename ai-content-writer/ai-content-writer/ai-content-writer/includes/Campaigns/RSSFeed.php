...
class RSSFeed {
	...
	/**
	 * Generate titles.
	 * This function is responsible for generating titles based on the provided RSS feed.
	 *
	 ...
	 */
	public static function generate_titles( $campaign_id, $posts_needed ) {
		if ( empty( $campaign_id ) || empty( $posts_needed ) ) {
			return false;
		}

		$feed_link = get_post_meta( $campaign_id, '_aicw_rss_feed_link', true );
		...
		// Suppress warnings and manually handle XML errors.
		...
		foreach ( $rss->channel->item as $item ) {
			...
			$exists_post_ids = aicw_get_posts_by_title( $title );
			$should_skip     = false;

			foreach ( $exists_post_ids as $post_id ) {
				if ( get_post_meta( $post_id, '_aicw_content_source_link', true ) === $link ) {
					$should_skip = true;
					break;
				}
			}
			...
			if ( $count >= $posts_needed ) {
				ai_content_writer()->create_log( sprintf( /* translators: 1: Number of posts generated. */ __( '%d posts titles and source links generated from RSS feed.', 'ai-content-writer' ), $count ), 'success', $campaign_id );
				break;
			}
		}
